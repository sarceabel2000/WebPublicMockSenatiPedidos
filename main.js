const URL =
  "https://8d28-2803-a3e0-1354-2fe0-b97e-a107-d39-8f5c.ngrok-free.app/pedidos";
...
function cargarPedidos(filtros = {}) {
  ...
  fetch(URL, {
    ...
    headers: {
      ...
      "x-api-key": "abcd1234",
      Authorization:
        "Bearer aD96N5kO1eNRM49pO2UwLTjA6Tz25mISjZ2VLYQjq7M0H1CsVP9tS7w7ki0GgTcg",
    },
    ...
    .then((pedidos) => {
      ...
      if (pedidos.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="9" style="text-align: center;">No se encontraron pedidos</td></tr>';
        return;
      }
      ...
      pedidos.forEach((p) => {
        ...
        <td><button class="btnDetalle" onclick='mostrarDetalle(${JSON.stringify(
          p
...
function mostrarDetalle(pedido) {
  ...
  document.getElementById("tituloPedido").innerText = `Detalle del Pedido ${
    pedido.number || "N/A"
  }`;
  document.getElementById("infoPedido").innerHTML = `
    ...
    <p><strong>Fecha Solicitud:</strong> ${new Date(
      pedido.orderDate
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("formFiltros").addEventListener("submit", (e) => {
    e.preventDefault();
    const inputNumberValue = document.getElementById("filtroNumber").value;
    const filtros = {
      number: inputNumberValue ? Number(inputNumberValue) : undefined,
      ...
      minProductos:
        parseInt(document.getElementById("filtroMinProductos").value) || 0,
      maxProductos:
        parseInt(document.getElementById("filtroMaxProductos").value) || 0,
    };
